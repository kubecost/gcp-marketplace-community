...
As of 2.4.0, the prometheus image is "latest" from chainguard.
I believe you need to edit that to use the old method:
```
  image:
...
You must do a global find in vscode for the old version and replace with the new version.
...
$BIN_FILE install --deployer=gcr.io/kubecost1/gcp-mp/cost-model/deployer:$DEPLOYERTAG  --parameters='{"name": "kubecost-myname", "namespace": "kubecost-myname"}'
### Clean up
kubectl delete application kubecost-myname -n kubecost-myname
...
### Build Deployer image and push it into GCR (Multi-arch image is not supported for the deployer)
helm dependency build chart/kubecost
docker build -t gcr.io/kubecost1/gcp-mp/ent/cost-model/deployer:$DEPLOYERTAG -f deployer/Dockerfile --push .
### Verification command:
$BIN_FILE verify --deployer=gcr.io/kubecost1/gcp-mp/ent/cost-model/deployer:$DEPLOYERTAG
## logs location
# /home/myuser/.mpdev_logs/
### test deployment
$BIN_FILE install --deployer=gcr.io/kubecost1/gcp-mp/ent/cost-model/deployer:$DEPLOYERTAG  --parameters='{"name": "kubecost-myname", "namespace": "kubecost-myname"}'
### Clean up
kubectl delete application kubecost-myname -n kubecost-myname
